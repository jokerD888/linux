...
int main(int argc, char* argv[]) 
{
    ...
    fds[0].fd = listenfd;
    fds[0].events = POLLIN | POLLERR;   /*设置事件*/
    fds[0].revents = 0;

    while (1) {
        ret = poll(fds, user_counter + 1, -1);  /*timeout为 - 1表阻塞*/
        if (ret < 0) {
            printf("poll failure\n");
            break;
        }
        ...
        for (int i = 0; i < user_counter + 1; ++i) {
            if (fds[i].fd == listenfd && fds[i].revents & POLLIN) {
                ...
                users[fds[i].fd] = users[fds[user_counter].fd];     //将最后的user_counter位置的连接换到i位置，随后--user_counter
                close(fds[i].fd);
